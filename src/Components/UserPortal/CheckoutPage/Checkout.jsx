...
const Checkout = () => {
  ...
  const handleSubmitAddress = async (e) => {
    ...
    try {
      ...
      setStatus(
        `Failed to save address: ${err.response?.data?.message || "Check your input."}`,
      );
    }
  };
  ...
  const handleDeleteAddress = async (id) => {
    ...
    try {
      ...
      setStatus(
        `Failed to delete address: ${err.response?.data?.message || "Unknown error"}`,
      );
      await fetchAddresses();
    }
  };
  ...
  const handlePayment = async () => {
    ...
    const finalShippingDetails = userAddresses.find(
      (addr) => addr._id === selectedAddressId,
    );
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (response) {
          ...
          try {
            ...
            if (verifyRes.data.success) {
              // Payment successful, now place the order in DB
              placeOrder(finalShippingDetails, "Razorpay");
            } else {
        ...
        prefill: {
          ...
          email: "", // Can fetch from user profile if available
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        try {
          ...
          console.debug("clearCart dispatch failed", e);
        }
        try {
          localStorage.removeItem("cartState");
          localStorage.removeItem("cart");
        } catch (e) {
          console.debug("failed clearing localStorage after order", e);
        }
        window.scrollTo(0, 0);
      }
    } catch (err) {
      ...
      alert(
        `Payment successful but failed to place order: ${err.response?.data?.message || err.message}`,
      );
    }
  };
          ...
          {userAddresses.map((address) => (
            <div
              ...
              style={{
                ...
                cursor: "pointer",
                backgroundColor:
                  selectedAddressId === address._id ? "#f5f3ff" : "#fff",
        ...
        <div className="checkout-container">
          ...
          <div className="summary-section">
            ...
            <div className="secure-payment">
              ...
              >
                <path
                  d="M12 11V14M12 14V17M12 17C12.5523 17 13 16.5523 13 16C13 15.4477 12.5523 15 12 15C11.4477 15 11 15.4477 11 16C11 16.5523 11.4477 17 12 17Z"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
